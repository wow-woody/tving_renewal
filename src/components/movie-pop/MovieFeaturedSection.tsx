...
import './MovieFeaturedSection.scss';
import { getMovieContents } from './getMovieContents';
...
const MovieFeaturedSection = () => {
  const list = useMemo(() => getMovieContents(), []);
  ...
  return (
    ...
    >
      <div className="section-header">
        <h2 className="section-title">영화 인기 프로그램</h2>
      ...
      <div className="enter-featured-body">
        <div className="featured-fixed">
          ...
          <div className="featured-info">
            ...
            {/* 영화 데이터 특성상 broadcast/season이 없을 수 있음 */}
            {activeItem.broadcast && <p className='broadcast'>{activeItem.broadcast}</p>}
            {activeItem.season && <p className='season'>{activeItem.season}</p>}
            {/* 영화에서는 year/time 표시가 유용 */}
            {activeItem.year && <p className='year'>{activeItem.year}</p>}
            {'time' in activeItem && (activeItem as any).time && (
              <p className='time'>{(activeItem as any).time}</p>
            )}
...
export default MovieFeaturedSection;
