...
export default function DramaDetailTving() {
    ...
    // 출연진 (최대 8명)
    const cast =
        (tvDetail as any).credits && Array.isArray((tvDetail as any).credits.cast)
            ? (tvDetail as any).credits.cast.slice(0, 8)
            : [];
    // 비슷한 콘텐츠 (예시: tvDetail.similar)
    const similar =
        (tvDetail as any).similar && Array.isArray((tvDetail as any).similar.results)
            ? (tvDetail as any).similar.results.slice(0, 8)
            : [];
    ...
    return (
        <div className="drama-detail-tving">
            ...
            <div className="drama-visual">
                <div
                    className="drama-visual-bg"
                    style={{
                        backgroundImage: tvDetail.backdrop_path
                            ? `url(https://image.tmdb.org/t/p/original${tvDetail.backdrop_path})`
                            : undefined,
                    }}
                />
            </div>
