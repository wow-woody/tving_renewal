...
const MyLive = ({ list }: Props) => {
    const navigate = useNavigate();
    const myLiveList = list.filter((item) => item.state === 'mylive');
    ...
    if (myLiveList.length === 0) return null;

    return (
        <section
            className="my-live"
            style={{ '--mylive-progress': `${progress}px` } as CSSProperties}
        >
            {/* 헤더 */}
            <div className="section-header">
                <h2>내 취향 딱! 인기 라이브</h2>
                <div className="thumb-controls">
                    <div className="mylive-pagination" ref={trackRef}>
                        <div className="pagenation-line" />
                        <div className="pointer-line" ref={barRef} />
                    </div>
                    <div className="mylive-nav">
                        <button
                            type="button"
                            ...
                            <div className="meta">
                                <div className="row">
                                    ...
                                    <p
                                        className="title"
                                        onClick={() =>
                                            navigate('/live', { state: { channelId: item.id } })
                                        }
                                    >
                                        {item.title}
                                    </p>
